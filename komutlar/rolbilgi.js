...
exports.run = async (bot, message, args) => {
    if (!message.guild.me.permissions.has("MANAGE_ROLES")) return message.channel.send("**__ROLLERİ YöNET__ Yetkim Olmadığı için Göstermiyorum!**");
    let role = message.mentions.roles.first() || args[0];
    if (!role) return message.channel.send('**Lütfen Bir Rol Gir!**')
    try {
    ...
    .setThumbnail(`https://hizliresim.com/Ggumc4`)
